        ...
        for (const auto& i : old_project) {	//扫描上一次产生的所有项目
            if (grammar_list[i.p_grammar].right.size() > i.point && isVN(grammar_list[i.p_grammar].right[i.point])) {
                ...
                for (j = 1; i.point + j < grammar_list[i.p_grammar].right.size(); j++) {
                    ...
                    if (p != firstba.cend()) {
                        //如果含有epsilon,则删除epsilon并加入后续follows集
                        firstba.erase(p);
                        continue;
                    }
                    else
...
)
{
    ...
    while (true) {
        ...
        else if (m_now.action == Action::reduction)
        {
            ...
            vector<Token> right_list(len);				//右部非终结符
            //移出栈
            while (len-- > 0) {
                SStack.pop();

                right_list[len] = TStack.top();				//为归约函数创建入口参数
                TStack.pop();
...
const Token& LR1_Parser::getErrorToken()				//返回出错信息
{
